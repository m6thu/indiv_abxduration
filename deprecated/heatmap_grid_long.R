...
mean_dur <- 14                           # antibiotic duration

######################### Heatmap Prototype (transmission vs prescription) ###########################

save_runs <- list()

bif_seq <- seq(0, 500, 10) 
repop.s_seq <- seq(0, 0.9, 0.01)

totalS <- matrix(NA, nrow = length(bif_seq), ncol = length(repop.s_seq))
totalsr <- matrix(NA, nrow = length(bif_seq), ncol = length(repop.s_seq))
totalSr <- matrix(NA, nrow = length(bif_seq), ncol = length(repop.s_seq))
totalsR <- matrix(NA, nrow = length(bif_seq), ncol = length(repop.s_seq))
...
for(bif in bif_seq){
    ...
    #                                       (pi_r1 < pi_r2 if being colonised with S protects colonisation by R)
    repop.s_count <- 1 
    for(repop.s in repop.s_seq){
        repop.s1 <- repop.s                        # probability of repopulation of s to become S 
        repop.s2 <- repop.s                          # probability of repopulation of sr to become SR 
        repop.s3 <- repop.s                       # probability of repopulation of sR to become sr
        repop.r1 <- repop.s*10                          # probability of repopulation of Sr to become sR 
        repop.r2 <- repop.s*10                          # probability of repopulation of sr to become sR 
        ...
        for(iter in 1:iterations){
            ...
            print(paste("iter:", iter, "bif:", bif_count, '-', bif, "repop.s", repop.s_count, '-', repop.s))
        ...
        totalS[bif_count, repop.s_count] <- mean(rowSums(iter_totalS)/iterations/n.bed)
        totalsr[bif_count, repop.s_count] <- mean(rowSums(iter_totalsr)/iterations/n.bed)
        totalSr[bif_count, repop.s_count] <- mean(rowSums(iter_totalSr)/iterations/n.bed)
        totalsR[bif_count, repop.s_count] <- mean(rowSums(iter_totalsR)/iterations/n.bed)
        
        repop.s_count <- repop.s_count + 1
        #print("end p.s loop")
    }
...
save.image("bifvsrepop_long.Rdata")

require(fields)

pdf("test.pdf")
par(mfrow=c(2,2))
image.plot(t(totalS), xlab="repop.s", ylab="bif", axes=F, col=rev(terrain.colors(100)))
title("totalS (long dur: 14 days)")
axis(1, at = (1:ncol(totalS)-1)/10, labels=as.character(repop.s_seq))
axis(2, at = (1:nrow(totalS)-1)/10, labels=as.character(bif_seq))
image.plot(t(totalsr), xlab="repop.s", ylab="bif", axes=F, col=rev(terrain.colors(100)))
title("totalsr")
axis(1, at = (1:ncol(totalsr)-1)/10, labels=as.character(repop.s_seq))
axis(2, at = (1:nrow(totalsr)-1)/10, labels=as.character(bif_seq))
image.plot(t(totalSr), xlab="repop.s", ylab="bif", axes=F, col=rev(terrain.colors(100)))
title("totalSr")
axis(1, at = (1:ncol(totalSr)-1)/10, labels=as.character(repop.s_seq))
axis(2, at = (1:nrow(totalSr)-1)/10, labels=as.character(bif_seq))
