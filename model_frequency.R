...
colo.table <- function(patient.matrix, los.array, total_prop, capacity_prop, prop_R, r_mean, K){
    ...
    total_capacity = rnorm(number_of_patients, log(capacity_prop*exp(K)))
    ...
    prop.r.ent= rexp(length(prop_R_id), 1/r_mean)
    prop.r.ent.norm= (prop.r.ent-min(prop.r.ent))/(max(prop.r.ent)-min(prop.r.ent))#proportion of R in total population of Enterobacteriaceae
    
    r_bact = rep(log(0), number_of_patients)
    r_bact[prop_R_id]= log(prop.r.ent.norm*exp(total_existing[prop_R_id])) #total number of resistant Enterobacteriaceae for each patient (log)
    r_bact[which(r_bact<0)] = -Inf
    ...
    for(i in 2:nrow(patient.matrix)){
        ...
        for(j in 1:ncol(patient.matrix)){
            if(is.na(R_table[i, j])){ # pick any; S and R should be filled in same slots
                ...
                # print(paste0('R',exp(R_table[i-1, j])))
                # print(paste0('S',exp(S_table[i-1, j])))
...
diff_prevalence <- function(n.bed, max.los, p.infect, cum.r.1, p.r.day1,
    ...
    for(iter in 1:iterations){
        ...
        iter_totalR.thres[, iter]=rowMeans(matrix(rowSums(df.R>=r_trans), ncol=timestep, byrow=T))
...
parameters_prevalence_freq <- c("n.bed", "max.los", "p.infect", "cum.r.1", "p.r.day1", 
                     ...
                     "pi_ssr", "r_mean", "r_growth", "r_trans", 's_growth',
                     "abx.s", "abx.r",
                     "meanDur")
