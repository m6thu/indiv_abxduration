...
plot_heatmap <- function (model = model, plots_data) {
  ...
  for (i in 1:length(plots_data)) {
    for (j in 1:(length(plots_data)-1)) {
      ...
      if (i == j) { #diagonal plots to be filled with white spaces 
      ...
      } else if (j == (length(plots_data)-1) & i != 1 & i != length(plots_data)) { #last column to keep y axis label on right
      ...
      } else if (j > 1 & j < length(plots_data) & i > 1 & i < length(plots_data)) { #plots in the middle to remove all axis labels 
      ...
      } else if (j > 1 & j < (length(plots_data)-1) & i == length(plots_data)){ #plots on the bottom to keep x axis labels 
      ...
      } else if (j != (length(plots_data)-1) & i == 1){ #plots on the top to keep x axis labels on top 
        ...
        plot = ggplot(outcome.df, aes(x, y)) +
          ...
          xlab(x_name)+
          theme_minimal()+
          scale_x_continuous(labels=scaleFUN, position = 'top') +
          theme(legend.position = 'none', 
                axis.title.x = element_text(size=font.size.axislab),
                axis.text.x = element_text(size=font.size),
                axis.title.y=element_blank(),
      ...
      } else if (j == (length(plots_data)-1) & i == 1){ #right top corner plot to keep all labels 
      ...
      } else if (j == (length(plots_data)-1) & i == length(plots_data)){ #right bottom corner plot to keep y label white
  ...
  combinedplot = ggarrange(plotlist = unlist(plots, recursive=FALSE), ncol = (length(plots_data)-1), nrow = length(plots_data))
