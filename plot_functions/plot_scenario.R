...
plot_scenario <- function(model, scenario) {
    ...
    if(model == "simple"){
        ...
        if (scenario == 'esbl') {
            para =  c(common.para$esbl, simple.para$esbl)
        } else {
            para =  c(common.para$cpe, simple.para$cpe)
        }
        ...
        matrixes.short = los.abx.table(n.bed=para$n.bed, n.day=para$n.day, max.los=para$max.los, 
        ...
        matrixes.long = los.abx.table(n.bed=para$n.bed, n.day=para$n.day, max.los=para$max.los, 
                                      p.infect=para$p.infect, p.r.day1=para$p.r.day1, cum.r.1=para$cum.r.1, 
        ...
        if (scenario == 'esbl') {
            para =  c(common.para$esbl, binary.para$esbl)
        } else {
            para = c(common.para$cpe, binary.para$cpe)
        }
        ...
        matrixes = los.abx.table(n.bed=para$n.bed, n.day=para$n.day, max.los=para$max.los, 
                                 p.infect=para$p.infect, p.r.day1=para$p.r.day1, cum.r.1=para$cum.r.1, 
                                 meanDur= para$short_dur, timestep=timestep)
        ...
        sumplot=ggplot(cumsumdata, aes(x=Time..days., y=cumsum, colour = dur))+
            geom_line()+
            labs(x = "Time (days)", y="Cumulative sum of R carriers")+
            scale_color_manual(values =  c('#A0495B', '#0096BC'), name='Treatment duration')+
            theme_bw()+
            theme(axis.text = element_text(size = base_size+2, colour = "grey50"), 
                  axis.title = element_text(size=base_size+2), 
        ...
        if (scenario == 'esbl') {
            para = c(common.para$esbl, frequency.para$esbl)
        } else {
            para = c(common.para$cpe, frequency.para$cpe)
        }
