...
gen_StartBact <- function(patient.matrix, prob_StartBact){
    ...
    prob_start_s <- 1-prob_start_S-prob_start_Sr-prob_start_sR-prob_start_sr
    
    #Generating a vector of random status with runif (change for other distribution)
    number_of_patients<- patient.matrix[length(patient.matrix)]
    Patient_StartBact <- runif(number_of_patients,0,1)
    Patient_StartBact[Patient_StartBact > prob_start_S+prob_start_Sr+prob_start_sR+prob_start_sr] <- 's'
    Patient_StartBact[Patient_StartBact <= prob_start_S+prob_start_Sr+prob_start_sR+prob_start_sr & Patient_StartBact>prob_start_S+prob_start_Sr+prob_start_sR] <- 'sr'
    Patient_StartBact[Patient_StartBact <= prob_start_S+prob_start_Sr+prob_start_sR & Patient_StartBact>prob_start_S+prob_start_Sr] <- 'sR'
    Patient_StartBact[Patient_StartBact <= prob_start_S+prob_start_Sr & Patient_StartBact>prob_start_S] <- 'Sr'
    ...
    for (i in 1:number_of_patients){
        for (j in 1:n.bed){
            array_StartBact[get1stdayofstay(i, j, patient.matrix), j] <- Patient_StartBact[i]
        }
    }
    # Populate each patient starting day with generated StartBact sampled
    
    #     for (i in 1:n.days){
    #         for (j in 1:n.bed){
    #             if(is.na(array_StartBact[i,j])){
    #                 array_StartBact[i, j] <- 0
    #             }else{
    #                 array_StartBact[i, j] <- array_StartBact[i, j]
    #             }
    #         }
    #     }
...
#output
colo_table_filled_short <- nextDay(bed_table= abx.short[[1]], array_LOS=array_LOS_short, treat_table=abx.short[[3]], colo_table=array_StartBact_short, pi_r1=pi_r1, mu1=mu1, mu2=mu2, pi_r2=pi_r2, repop.r1 = repop.r1, repop.r2 = repop.r2, repop.s1 = repop.s1, repop.s2 = repop.s2)
colo_table_filled_long <- nextDay(bed_table= abx.long[[1]], array_LOS=array_LOS_long, treat_table=abx.long[[3]], colo_table=array_StartBact_long, pi_r1=pi_r1, mu1=mu1, mu2=mu2, pi_r2=pi_r2, repop.r1 = repop.r1, repop.r2 = repop.r2, repop.s1 = repop.s1, repop.s2 = repop.s2)
