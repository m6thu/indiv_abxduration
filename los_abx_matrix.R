...
seq_along_admissiondates <- function(x){ return(1:x)}
...
los.abx.table <- function(n.bed, n.day, max.los,
  ...
  abx_days.total = round(rtnorm(200, mean = meanDur, lower = 1)) #200 arbituary number 
  
  abx_days.s = abx_days.total[1:100]
  abx_days.r = abx_days.total[101:200]
  
  no.abx.r.day1 = round(n.patient.max * p.infect * p.r.day1) #number of broad spectrum spectrum antibiotics for community acquired infection 
  no.abx.s.day1 = round(n.patient.max*p.infect) - no.abx.r.day1 #number of narrow spectrum antibiotics for community acquired infection 
  
  id.abx.r.day1 = sample(patient.id, no.abx.r.day1) #id of patients on broad spectrum antibiotics on day 1
  id.abx.s.day1 = sample(patient.id[-id.abx.r.day1], no.abx.s.day1) #id of patients on narrow spectrum antibiotics on day 1
  ...
  for (i in 1:length(all_los)){ # for every patient 
    ...
    # p=ecdf(probs.normalized) #cumulative distribution
    prob.r.after = p((1/cum.r.1) * all_admission_days[[i]]) # get the cumulative probability for each day
