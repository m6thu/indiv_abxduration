...
# p.infect = probability of infection of both S and R
los.abx.table <- function(n.bed, n.day, mean.max.los,
    ...
    all_abx[id.abx.r.day1] <- repVec(2, times = sample(abx_days.r,length(id.abx.r.day1), replace = T))
    all_abx[id.abx.s.day1] <- repVec(1, times = sample(abx_days.s,length(id.abx.s.day1), replace = T))
