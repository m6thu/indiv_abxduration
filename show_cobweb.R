...
##Spearman's rank correlation measure to test for non-monotonicity 
sampled_parameter_combi<-LHS.output$data
outputs<-LHS.output$res
spearman<-c()
hoeff=c()
for (i in 1:ncol(sampled_parameter_combi)){
    #square of Spearman's rank correlation
    spearman[i]=cor(rank(outputs), rank(sampled_parameter_combi[,i]))^2
    #Hoeffding's D
    hoeff[i]=hoeffd(x=as.matrix(sampled_parameter_combi[,i]), y=outputs)$D[1,2]
}
corr.table=data.frame(parameters=colnames(sampled_parameter_combi), spearman=spearman, hoeffD=hoeff)
#low squared Spearman's rank correlation means non-monotonicity
#low Hoeffding's D value means parameters and outputs are not correlated in non-monotonic way 
...
plotscatter(LHS.output)

#3. partial correlation coefficient measures how strong are the inear associations between the result 
# and each input parameter, after removing the linear effect of the other parameters(CI generated by bootstrapping)
plotprcc(LHS.output, ylab = 'Partial Correlation coefficients')

pic(LHS.output, nboot=40) #pic (partial inclination coefficient) is the sensitivity" of the model response 
# in respect to each parameter, represent the beta terms in y = alpha + beta*x regressions, 
...
#check.LHS.output <- LHS(modelRun.output, factors.output, 250, q.output, q.arg.output, cl=cl)
(mySbma <- sbma(LHS.output, check.LHS.output))
