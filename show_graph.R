...
if (model=='simple') {
    ...
    prevalence <- function(n.bed, mean.max.los, 
        ...
        totalR = mean(rowSums(iter_totalR[ceiling(n.day*1/7):nrow(iter_totalR), ,drop=FALSE])/iterations/n.bed)
    ...
    prevalence <- function(n.bed, mean.max.los, 
                           ...
                           bif, pi_r2, repop.s1, repop.s2, repop.r1, repop.r2,
                           mu1, mu2, mu.r, abx.s, abx.r, 
                           p.infect, cum.r.1, p.r.day1, meanDur){
        ...
        for(iter in 1:iterations){
            ...
            colo.matrix_filled_iter = nextDay(patient.matrix=patient.matrix, abx.matrix=abx.matrix, colo.matrix=colo.matrix, 
                                              pi_r2=pi_r2, bif=bif, mu1=mu1, mu2=mu2, mu.r=mu.r, repop.r1=repop.r1, repop.r2=repop.r2,
        ...
        totalsR = mean(rowSums(iter_totalsR[ceiling(n.day*1/7):nrow(iter_totalsR),, drop=FALSE])/iterations/n.bed)
        totalr_or_R = mean(rowSums(iter_totalr_or_R[ceiling(n.day*1/7):nrow(iter_totalr_or_R),, drop=FALSE])/iterations/n.bed)
    
        # print elapsed time
        new = Sys.time() - old # calculate difference
        print(new) # print in nice format
        
        return(matrix('totalsR',  'totalr_or_R',
                      totalsR, totalr_or_R, 
                      nrow=2))
    ...
    prevalence <- function(n.bed, mean.max.los, p.infect, cum.r.1, p.r.day1,
        ...
        totalR_no = mean(rowSums(iter_totalR.no)/iterations/n.bed)
        totalR_thres = mean(rowSums(iter_totalR.thres)/iterations/n.bed)
        ...
        return(matrix('totalR_no', 'totalR_thres',
                      totalR_no, totalR_thres, 
                      nrow=2))
    }
...
dat=cbind.data.frame(samples.matrix, out) #dataframe with columns of parameter values + outcome values 
...
# calculating mean
meanY= c(mean(dat[1:50,]$Y), mean(dat[51:100,]$Y), mean(dat[101:150,]$Y))
...
ggplot(dat,aes(x=Time,y=Y)) + 
    ...
    #label 
    annotate(geom="text", x=2.2, y=meanY[2], label=paste(para, '=', min.para))+
    annotate(geom="text", x=2.2, y=meanY[3], label=paste(para, '=', max.para))+
    annotate(geom="text", x=0.8, y=meanY[1], label=paste(para, '=', min.para))+
    ylab("Number of patients carrying R/bed/month")+
    theme_minimal() 
